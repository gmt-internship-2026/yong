...
작성: 2026-07-08. 맥(Apple Silicon) 기준, 윈도우는 각 단계의 표기 참고.
**Jetson 보드 셋업은 이 문서가 아니라 [JETSON_SETUP.md](JETSON_SETUP.md)를 볼 것.**

## 1. 프로그램 설치 (PC에 딱 3개)

| 프로그램 | 확인 방법 | 설치 |
|---|---|---|
| Python 3.10 이상 | 터미널에서 `python3 --version` | 맥: 기본 내장(3.9 이하면 [python.org](https://www.python.org/downloads/)) / 윈도우: python.org에서 설치(설치 시 "Add to PATH" 체크) |
| Git | `git --version` | 맥: 기본 내장 / 윈도우: [git-scm.com](https://git-scm.com) |
...
③에서 손바닥을 카메라에 펴 보이고 1초 유지 → 화면에 `palm_stop` 이벤트가 뜨면 전체 파이프라인 정상.
...
| 카메라가 안 열림 | 시스템 설정 → 개인정보 보호 → 카메라에서 터미널/VS Code 허용. 외장 캠이면 `config.yaml`의 `camera.device_id`를 1, 2로 변경 |
