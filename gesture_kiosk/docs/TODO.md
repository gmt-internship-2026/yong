...
- [x] **(2026-07-16) 판정 계기판 + 임계 완화** — 실기 인식 저하(사용자 보고) 원인이
      ...
      추정 → min_dist 0.8/0.5·window 0.8초로 완화 + 판정 내부값(스케일·진행도·장전·
      꾸벅 기준선)을 /data·화면·데모UI에 노출 — 이후 튜닝은 수치 근거로. 테스트 48건
...
- [ ] **№1 제스처 스펙 확정** — 2026-07-15 범용 설계 스펙(쓸기 4방향 + 고개 꾸벅 2회)으로 재구현.
