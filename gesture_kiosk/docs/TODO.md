...
- [ ] **№1 제스처 스펙 확정** — 2026-07-16 토크백/보이스오버 정렬 스펙(사용자 결정):
      좌/우 쓸기=이동, 위/아래 1회=확인(read_focus)/선택, 위/아래 2연속=홈/이전(안전장치).
      ⚠ 고개 꾸벅 제거로 **양팔이 모두 없는 사용자의 선택 수단 부재** — UI 자동 순회
      (스캐닝: 포커스 자동 이동 + 아무 동작으로 선택) 도입 여부 회사 협의 필요.
