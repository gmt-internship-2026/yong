...
echo  사용법: install.bat          ^(기본 설치 — torch 백엔드^)
echo         install.bat /engine  ^(설치 + TensorRT 엔진 빌드^)
echo ============================================================
...
if exist wheelhouse (
    ...
    echo [INFO] 온라인 설치 — CUDA 12.8 torch (RTX 50시리즈 포함 지원)
...
REM ---- 4) 모델 가중치 (오프라인 번들 우선) --------------------
if exist bundle_models\weights (
    echo [INFO] 오프라인 모델 복사 — bundle_models\ 사용
    xcopy /y /q bundle_models\weights models\weights\ >nul
) else (
...
REM ---- 6) TensorRT 엔진 빌드 (선택) ---------------------------
if /I "%~1"=="/engine" (
    echo [INFO] TensorRT 설치 + 엔진 빌드 — 이 PC 전용 (약 5~15분)
    if exist wheelhouse (
        pip install --no-index --find-links wheelhouse tensorrt || goto :pip_fail
    ) else (
        pip install tensorrt || goto :pip_fail
    )
    python scripts\build_engine.py || exit /b 1
    echo [다음] configs\config.yaml에서 model.backend: engine 으로 바꾸면 엔진으로 돕니다
)
